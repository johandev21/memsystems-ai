...
export class NoteTypeService {
	...
	async update(userId: string, noteTypeId: string, input: UpdateNoteTypeInput) {
		...
		if (input.fieldsSchema !== undefined) {
			...
			const existingFields = (nt.fieldsSchema as FieldSchema[]).map((f) => f.name);
	...
	private validateCardTemplates(templates: CardTemplate[]) {
		...
		for (let i = 0; i < templates.length; i++) {
			...
			if (!t.name || !t.name.trim()) {
				throw new BadRequestError(`Card template at index ${i} must have a name`);
			}
			if (!t.front || !t.front.trim()) {
				throw new BadRequestError(`Card template "${t.name}" must have a front template`);
			}
			if (!t.back || !t.back.trim()) {
				throw new BadRequestError(`Card template "${t.name}" must have a back template`);
			}
		}
	}
