...
export class NoteService {
	...
	async create(userId: string, input: CreateNoteInput) {
		...
		const fieldSchemas = nt.fieldsSchema as Array<{ name: string; required?: boolean }>;
		...
		for (const key of Object.keys(input.fieldValues)) {
			if (!fieldNames.includes(key)) {
				throw new BadRequestError(`Unknown field "${key}" for note type "${nt.name}"`);
			}
		}
	...
	private async createCardsForNote(note: typeof notes.$inferSelect) {
		const nt = await this.fetchNoteType(note.noteTypeId);
		const templates = nt.cardTemplates as Array<{ name: string; front: string; back: string }>;
