...
describe("AiService.searchWeb", () => {
  it("rejects models that do not support web search", async () => {
    ...
    vi.spyOn(aiService, "getProviderForModel").mockResolvedValue(provider as any);
...
describe("reconcileSearchSources", () => {
    ...
    toolResults: [
      {
        ...
        output: {
          ...
          sources: [
            { type: "url", url: "https://plato.stanford.edu/entries/epistemology/" },
  ...
  it("keeps only sources the model curated from real search results", () => {
    ...
    expect(sources[0].url).toBe("https://plato.stanford.edu/entries/epistemology/");
  });
