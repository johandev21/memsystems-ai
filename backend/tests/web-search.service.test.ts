...
function createSourcesService() {
  ...
  return { db, sourcesService, notebooksService, webScraperService };
}
...
function createWebSearchService(
  ...
  const aiService = {
    searchWeb: vi.fn().mockResolvedValue({
      ...
      sources: [
        { title: "Philosophy Basics", url: "https://philosophybasics.com", description: "Intro" },
        { title: "Stanford Encyclopedia", url: "https://plato.stanford.edu", description: "Deep dive" },
      ],
    }),
  } as any;
  const service = new WebSearchService(notebooksService, sourcesService, aiService);
...
describe("WebSearchService", () => {
  it("search returns candidates minus already-added URLs", async () => {
    ...
    const { service, aiService } = createWebSearchService(sourcesService, notebooksService);
  ...
  it("import scrapes and persists each candidate as an ai_search source", async () => {
    const { sourcesService, notebooksService, webScraperService } =
      createSourcesService();
    const user = await seedUser();
    const notebook = await seedNotebook(user.id);

    const { service } = createWebSearchService(sourcesService, notebooksService);
    ...
    expect(webScraperService.scrapeUrl).toHaveBeenCalledTimes(2);
  ...
  it("import marks duplicate URLs and skips re-scraping", async () => {
    ...
    expect(webScraperService.scrapeUrl).toHaveBeenCalledTimes(1);
