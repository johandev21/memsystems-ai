...
const MODELS = [
  { id: "sonnet-3.5", name: "Claude 3.5 Sonnet", provider: "Anthropic", desc: "Anthropic's most intelligent model", cost: "$$$", icon: Layers, locked: true },
  { id: "gpt-4o", name: "GPT-4o", provider: "OpenAI", desc: "OpenAI's frontier model", cost: "$$$", icon: Cpu, locked: true },
  { id: "gemini-1.5-pro", name: "Gemini 1.5 Pro", provider: "Google", desc: "Google's best model for long context", cost: "$$", icon: Sparkles, locked: false },
  { id: "claude-haiku", name: "Claude 3 Haiku", provider: "Anthropic", desc: "Lightning-fast responses", cost: "$", icon: Layers, locked: false },
];
    ...
    <div className="flex flex-1 h-full w-full flex-col">
      ...
      <div className="flex-1 overflow-y-auto pb-4 pt-12 lg:pt-20">
        <div className="mx-auto flex w-full max-w-3xl flex-col items-center px-6">
          ...
          {/* Prompts List */}
          <div className="w-full max-w-2xl relative min-h-[220px]">
      ...
      <div className="w-full shrink-0 px-6 pb-6 pt-2">
              ...
              >
                <div className="flex flex-col">
                  ...
                  <div className="flex items-center justify-between bg-muted/30 px-4 py-3 border-b border-border/50">
                    ...
                    <Button size="sm" className="h-7 rounded-full text-xs px-4 bg-primary text-primary-foreground hover:bg-primary/90">
                      Upgrade
                    </Button>
                  </div>
                          ...
                          >
                            <div className="flex w-full items-center justify-between">
                              <span className="flex items-center gap-2 text-sm font-medium text-foreground">
                                {model.name}
                                <span className="text-xs text-emerald-600 dark:text-emerald-400">{model.cost}</span>
                                {model.id === "sonnet-3.5" && <Star className="h-3 w-3 fill-amber-400 text-amber-400" />}
                              </span>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-muted-foreground line-clamp-1">{model.desc}</span>
                            </div>
                          </button>
                        ))}
                      </div>
        ...
        <div className="mt-4 text-center">
          <p className="text-xs text-muted-foreground/70">
            Make sure you agree to our <a href="#" className="underline hover:text-muted-foreground">Terms</a> and our <a href="#" className="underline hover:text-muted-foreground">Privacy Policy</a>
          </p>
        </div>
        </div>
      </div>
