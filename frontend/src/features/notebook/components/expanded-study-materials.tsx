...
function ResourceCard({
  ...
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        "flex items-center justify-center gap-2 h-11 px-5 rounded-full w-full text-foreground border-0",
    ...
    >
      <resource.icon className="h-4.5 w-4.5 shrink-0 opacity-90" strokeWidth={1.8} />
      <span className="text-xs font-semibold tracking-wide">{resource.label}</span>
    </button>
  );
}
