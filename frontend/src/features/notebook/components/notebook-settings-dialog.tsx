...
import {
	...
	removeNotebookBannerFn,
	updateNotebookFn,
	uploadNotebookBannerFn,
} from "#/lib/notebooks";
	...
	const handleSave = async () => {
		...
		try {
			...
			if (fieldsChanged || shouldUpdateFocalPoint) {
				...
				promises.push(updateNotebookFn({ data: body }));
			}
			if (bannerFile) {
				...
				promises.push(uploadNotebookBannerFn({ data: formData }));
			} else if (bannerRemoved && notebook.bannerUrl) {
				promises.push(removeNotebookBannerFn({ data: notebookId }));
			}
