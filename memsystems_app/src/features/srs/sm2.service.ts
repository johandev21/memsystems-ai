...
export function sm2(
  ...
  if (grade === REVIEW_GRADES.Again) {
    ...
    return {
      ...
      dueAt: new Date(
        now.getTime() + learningSteps[0].intervalMinutes * 60 * 1000,
      ),
    };
  }
  ...
  if (card.state === "new") {
    if (grade === REVIEW_GRADES.Hard) {
      const intervalMinutes = Math.round(
        learningSteps[0].intervalMinutes * 1.2,
      );
      return {
        state: "learning",
    ...
    if (grade === REVIEW_GRADES.Easy) {
      ...
      return {
        ...
        lapses: card.lapses,
        dueAt: new Date(
          now.getTime() + learningGraduateIntervalDays * 24 * 60 * 60 * 1000,
        ),
      };
    }
  }
  ...
  if (card.state === "learning") {
    ...
    if (grade === REVIEW_GRADES.Hard) {
      const intervalMinutes = Math.round(
        learningSteps[Math.max(0, nextStepIndex - 1)].intervalMinutes * 1.2,
      );
      return {
        state: "learning",
        easinessFactor: calculateNewEF(card.easinessFactor, grade),
